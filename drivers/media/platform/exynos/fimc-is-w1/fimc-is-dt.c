...
static int parse_dvfs_data(struct exynos_platform_fimc_is *pdata, struct device_node *np)
{
	...
	DT_READ_U32(np, "rear_camcording_int", pdata->dvfs_data[FIMC_IS_SN_REAR_CAMCORDING][FIMC_IS_DVFS_INT]);
	DT_READ_U32(np, "rear_camcording_cam", pdata->dvfs_data[FIMC_IS_SN_REAR_CAMCORDING][FIMC_IS_DVFS_CAM]);
	DT_READ_U32(np, "rear_camcording_mif", pdata->dvfs_data[FIMC_IS_SN_REAR_CAMCORDING][FIMC_IS_DVFS_MIF]);
	DT_READ_U32(np, "rear_camcording_i2c", pdata->dvfs_data[FIMC_IS_SN_REAR_CAMCORDING][FIMC_IS_DVFS_I2C]);
...
int fimc_is_sensor_parse_dt(struct platform_device *pdev)
{
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 2, gpio_cam_en, 0, NULL, PIN_OUTPUT_HIGH);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 3, gpio_reset, 0, NULL, PIN_RESET);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 4, gpio_none, 0, "ch", PIN_FUNCTION);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 5, gpio_comp_en, 0, NULL, PIN_OUTPUT_HIGH);
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 6, gpio_comp_rst, 0, NULL, PIN_RESET);
#if defined(CONFIG_SOC_EXYNOS5430)
	if ((id == SENSOR_POSITION_REAR) && (board_rev == 2)) {
		SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 7, gpio_none, 0, "af", PIN_FUNCTION);
	}
#else
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 7, gpio_none, 0, "af", PIN_FUNCTION);
#endif
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, 8, gpio_none, 0, NULL, PIN_END);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_OFF, 8, gpio_none, 0, NULL, PIN_END);

	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, 0, gpio_none, 0, "cam_core", PIN_REGULATOR_ON);
	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, 1, gpio_none, 0, "cam_sensor", PIN_REGULATOR_ON);
	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, 2, gpio_cam_en, 0, NULL, PIN_OUTPUT_HIGH);
	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, 3, gpio_reset, 0, NULL, PIN_OUTPUT_LOW);
	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, 4, gpio_standby, 0, NULL, PIN_OUTPUT_HIGH);
	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_ON, 5, gpio_none, 0, NULL, PIN_END);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_OFF, 3, gpio_cam_en, 0, NULL, PIN_OUTPUT_LOW);
	SET_PIN(pdata, SENSOR_SCENARIO_VISION, GPIO_SCENARIO_OFF, 4, gpio_none, 0, "cam_core", PIN_REGULATOR_OFF);
